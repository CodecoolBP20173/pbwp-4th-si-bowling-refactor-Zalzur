def score(game):
    ...
    for i in range(len(game)):
        # Adds the value of the character to score:
        if game[i] == '/':
            last_frame_half = get_value(game[i-1])
            score += 10 - last_frame_half
        else:
            score += get_value(game[i])
        # If it's a strike or a spare adds the bonus points to score:
        if frame < 10 and get_value(game[i]) == 10:
            score = handle_bonus_points(game, i, score)
